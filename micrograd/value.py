...
class Value:
    ...
    def __init__(self, data, _children=(), _op=''):
        self.data = data
        self.grad = 0
        self._backward = lambda: None # computes gradients with chain rule
    ...
    def __pow__(self, other): # other is an int/float
        assert isinstance(other, (int,float)), "supports int/float only"
    ...
    def backward(self): # runs topological sort and computes gradients on the reverse
        topo = []
        visited = set()
        def build_topo(v):
            if v not in visited:
